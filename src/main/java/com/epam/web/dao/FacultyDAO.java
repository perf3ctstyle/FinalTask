...
public class FacultyDAO extends AbstractDao<Faculty> implements Dao<Faculty> {
    ...
    private static final String FIND_LIMITED_NUMBER_OF_FACULTIES = "SELECT * FROM FACULTY LIMIT ?, ?";
    ...
    public List<Faculty> findLimitedNumberOfFaculties(int offset, int numberOfRecords) throws DaoException {
        return executeQuery(FIND_LIMITED_NUMBER_OF_FACULTIES, offset, numberOfRecords);
    }

    @Override
    protected void create(Faculty faculty) throws DaoException {
        executeUpdate(CREATE_FACULTY, faculty.getId(), faculty.getDescription(), faculty.getAdmissionPlan());
    }

    @Override
    protected void update(Faculty faculty) throws DaoException {
        executeUpdate(UPDATE_FACULTY, faculty.getId(), faculty.getDescription(), faculty.getAdmissionPlan());
    }
}
